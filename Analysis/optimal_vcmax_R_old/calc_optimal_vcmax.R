...
calc_optimal_vcmax <- function(tg_c = 25, z = 0, vpdo = 1, cao = 400, 
                               paro = 800, q0 = 0.257, theta = 0.85, beta = 146){
  ...
  km <- calc_km_pa(tg_c, z)                
	...
	chi <- calc_chi(tg_c, z, vpdo, cao, beta)   
	ci <- chi * ca # Pa
	...
	# output
	results <- as.data.frame(cbind(tg_c, z, vpdo, cao, paro, q0, theta, c, par, patm, ca, vpd, beta, chi, ci, km, 
	                               gammastar, omega, m, mc, omega_star, vcmax, jvrat, jmax))
	
	colnames(results) <- c('tg_c', 'z', 'vpdo', 'cao', 'paro', 'q0', 'theta', 'c', 'par', 'patm', 'ca', 'vpd', 'beta', 'chi', 'ci', 'km', 
	                       'gammastar', 'omega', 'm', 'mc', 'omega_star', 'vcmax', 'jvrat', 'jmax')
