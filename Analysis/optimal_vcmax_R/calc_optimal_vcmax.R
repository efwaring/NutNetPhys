...
calc_optimal_vcmax <- function(pathway = "C3", tg_c = 25, z = 0, vpdo = 1, cao = 400, oao = 209460,
                               paro = 800, q0 = 0.257, theta = 0.85, beta = 146, chi = NA, lma = NA, f = 0.5){
  ...
  if(pathway == "C3"){
    ...
    chi <- ifelse(!is.na(chi), chi, calc_chi(temp = tg_c, z = z, vpdo = vpdo, cao = cao, beta = beta))
    ...
    Ac <- vcmax * mc
      ...
      given_chi <- given_chi(chi) # returns yes or no based on presence of chi value
      # fill in missing chi values if chi is known
      chi <- ifelse(!is.na(chi), chi, calc_chi_c4(ca = cao, temp = tg_c, vpd = vpdo, z = z))
      # calculate chi if unknown
  ...
  lma <- ifelse(!is.na(lma), lma, calc_lma(f = f, par = paro, temperature = tg_c, vpd_kpa = vpdo, z = z, co2 = cao))
